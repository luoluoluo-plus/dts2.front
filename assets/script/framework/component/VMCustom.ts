...
export default class VMCustom extends VMBase {
    ...
    checkComponentState() {
        ...
        if (!this._watchComponent) { Log.error('未设置需要监听的组件'); return; }
        if (!this.componentProperty) { Log.error('未设置需要监听的组件 的属性'); return; }
        if (this.componentProperty in this._watchComponent === false) { Log.error('需要监听的组件的属性不存在'); return; }
